...
public class GClass841 : GClass839
{
	public GClass841(GClass823 gclass823_1)
	{
	...
	public override List<Rectangle> vmethod_17()
	{
		...
		Rectangle value = GClass841.list_0[0];
	...
	{
		gate_0 = null;
		List<KeyValuePair<int, Gate>> list = base.C.Map.Gates.Where(new Func<KeyValuePair<int, Gate>, bool>(this.method_23)).OrderBy(new Func<KeyValuePair<int, Gate>, int>(GClass841.<>c.<>c_0.method_0)).ToList<KeyValuePair<int, Gate>>();
		if (list.Count != 2)
		{
			return null;
		}
		CollidableRect collidableRect = base.C.Map.Collidables.FirstOrDefault(new Func<KeyValuePair<string, ICollidable>, bool>(GClass841.<>c.<>c_0.method_1)).Value as CollidableRect;
