...
public static class GClass785
{
	public static string smethod_0(string string_0, string string_1)
	{
		...
		return Convert.ToBase64String(GClass785.smethod_2(unicode.GetBytes(string_0), unicode.GetBytes(string_1)));
	}

	public static string smethod_1(string string_0, string string_1)
	{
		Encoding unicode = Encoding.Unicode;
		return unicode.GetString(GClass785.smethod_2(unicode.GetBytes(string_0), Convert.FromBase64String(string_1)));
	}

	public static byte[] smethod_2(byte[] byte_0, byte[] byte_1)
	{
		return GClass785.smethod_5(byte_0, byte_1).ToArray<byte>();
	}
	...
	{
		byte[] array = Enumerable.Range(0, 256).Select(new Func<int, byte>(GClass785.<>c.<>c_0.method_0)).ToArray<byte>();
		int i = 0;
		int num = 0;
		while (i < 256)
		{
			num = (num + (int)byte_0[i % byte_0.Length] + (int)array[i] & 255);
			GClass785.smethod_6(array, i, num);
			i++;
		}
		return array;
	}
	...
	{
		GClass785.<>c__DisplayClass5_0 CS$<>8__locals1 = new GClass785.<>c__DisplayClass5_0();
		CS$<>8__locals1.byte_0 = GClass785.smethod_4(byte_0);
